...
class T3RExtractor:
    ...
    def assay_category(self) -> AssayCategory:
        val = self._doc["DirectControlAssayResultsFile"]["AssayData"]["AssayTemplate"]["Category"]
        return AssayCategory(val)




class UVMetricPKaT3RExtractor(T3RExtractor):
    """Class to handle results file from UVMetric pKa Experiment"""
    EXPECTED_ASSAY_CATEGORY = AssayCategory.PKA

    @cached_property
    def _mean_dpas_result(self) -> dict:
        try:
            return self._doc["DirectControlAssayResultsFile"]["ProcessedData"]["FastDpasMeanResult"]
    ...
    def predicted_pKa(self) -> PkaResult:
        data = self._doc["DirectControlAssayResultsFile"]["ProcessedData"]["PhMetricModel"]["Sample"]["Pka"]
        predicted_type = PkaType(data["PkaType"]["Value"])
        predicted_value = float(data["PkaValue"]["Value"])
        prediction_source = data["PkaValue"]["Source"]
        return PkaResult(value=predicted_value, std=None, pka_type=predicted_type, source=prediction_source)


class LogPT3RExctractor(T3RExtractor):
    ...
    def logp_result(self) -> LogPResult:
        data = self._doc["DirectControlAssayResultsFile"]["ProcessedData"]["MultisweepPhMetricResult"]
        rmsd = float(data["Rmsd"])
        value_list = data["MultisweepPhMetricLevelResult"]["SampleValues"]["Logp"]
        # NEED TO VERIFY THIS WILL BE CORRECT
        value = max(float(value) for value in value_list) 
        return LogPResult(value=value, rmsd=rmsd)

    @property
    def logp_solvent(self) -> str:
        return self._doc["DirectControlAssayResultsFile"]["AssayData"]["AssayTemplate"]["Settings"]["PartitionType"]["Value"]["#text"]